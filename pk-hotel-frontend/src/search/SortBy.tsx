...
interface SortByProps {
  onSortChange: () => void; // Function to handle sort change
  sortOrder: "asc" | "desc"; // Current sort order state
}

export const SortBy = ({ onSortChange, sortOrder }: SortByProps) => {
  const arrowIcon = (
    <svg
      width="24"
      height="24"
  ...
  return (
    <button className="sort-by-button" onClick={onSortChange}>
      <span className="sort-text">Sort by: </span>
      <span className="arrows">{arrowIcon}</span>
    </button>
  );
};
